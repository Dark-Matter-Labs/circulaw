    ...
    <div key = {index} className='block sm:ml-0 lg:ml-28 pb-8 sm:pb-10'>
      ...
      <div className='inline-block'>
        {law?.thema === 'houtbouw' && 
          ...
          <span className='inline-block pl-4 font-openSans casus'>{law?.thema}</span>
                {law?.extraContent && law?.extraContent.map((content) => (
                (
                  <span key={content} className='ml-2 p-1 text-normal rounded font-semibold text-sm bg-green2 text-white no-underline'>
                    {content}
                  </span>
                )))}  
      </div>
      ...
      <div className='block newlineDisplay twoLines font-manrope font-normal font-base mb-4'>
          <Link href={'/measures/' + law.slug.current} key={law.slug.current} passHref>
            <a>
              <p className='max-w-xs sm:max-w-5xl'>
                {law.introText}
              </p>
              </a>
          </Link>
        ...
        <div className='grid grid-cols-1 lg:flex space-x-0 lg:space-x-8 space-y-4 sm:space-y-0  py-2 sm:py-0'>
                <div className='flex-2 mr-5 text-normal font-openSans text-xs text-black1 sm:text-gray-400 '>
                  <span className='block-inline flex items-center'>
                    Juridische invloed:{' '}<span className='text-black uppercase pl-1'>{law.juridischInvloed}</span>
                  </span>
                </div>

                <div className='flex-2 mr-5 text-normal font-openSans text-xs text-black1 sm:text-gray-400 '>
                  <span className='block-inline flex items-center'>
                    Juridische invloed:{' '}<span className='text-black uppercase pl-1'>{law.juridischHaalbaarheid}</span>
                  </span>
                </div>
