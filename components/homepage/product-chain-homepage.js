  ...
  return (
    <>
      <div className='relative flex w-full p-3'>
        ...
        <ul className='no-scrollbar relative z-20 flex h-full w-full snap-mandatory flex-row gap-x-6 overflow-scroll'>
          {pcData?.map((chain, id) => (
            <Link
              key={id}
              href={`/${chain.slug}`}
              className='group flex min-w-[229px] max-w-[256px] grow'
            >
