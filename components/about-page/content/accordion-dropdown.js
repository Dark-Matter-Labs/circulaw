    ...
    <div className='mb-[60px] sm:mb-[120px]'>
      <Disclosure>
        {({ open }) => (
          <>
            <DisclosureButton
              className={`group flex w-full items-center justify-between border-black py-4 sm:py-10 pr-2 sm:pr-6 text-gray-800 ${
                open
                  ? 'border-b-none border-t-2' // Top border only when open
                  : 'border-y-2' // Full border when closed
              }`}
            >
              <h4 className='heading-2xl-semibold sm:heading-3xl-semibold text-left'>{data.title}</h4>
              <IconChevronDown
            ...
            <DisclosurePanel className='border-b-2 border-black py-4 sm:py-10'>
              <div className='grid w-full grid-cols-1 sm:grid-cols-2 justify-between gap-y-6 sm:gap-y-10'>
                {data.content.map((item, index) => (
                  <div key={index} className=' max-w-[500px]'>
                    <div className='mb-2'>
                      <h3 className='heading-2xl-semibold sm:heading-3xl-semibold text-black'>{item?.title}</h3>
