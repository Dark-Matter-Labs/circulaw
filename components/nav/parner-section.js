...
export default function ParnerSection({ partners, partnerType }) {
  return (
    <>
      <div className='heading-xl-semibold my-4 pl-3 text-green-100 sm:pl-6 lg:pl-8'>
        {partnerType}
      </div>
      <div
        className={`${
          partnerType === 'Financieringspartners'
            ? ''
            : 'mb-8 border-b border-green-100 sm:mb-12 sm:pb-8'
        ...
        {partners?.map((partner) => (
          <Fragment key={partner.partnerName}>
            <div
              ...
              className={`col-span-1 flex items-center justify-center border-r-0 border-green-100 px-4 py-4 sm:border-r lg:px-8 lg:py-4 ${bordersDesktop} ${bordersSmall}`}
            >
