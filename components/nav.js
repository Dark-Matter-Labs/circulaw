...
export default function Nav(props) {
  ...
  if (router.pathname === '/en') {
    ...
    return (
      ...
      <Disclosure as='nav' className={`${router.pathname !== '/' ? 'sticky top-0 z-40 bg-black-white-200 shadow-lg' : ''}`}> 
        {({ open }) => (
          <>
            <div className='lgNav:pb-4 global-margin'>
              ...
              <div className=''>
                <div className='inset-y-0 float-right flex items-center lgNav:hidden'>
                  {/* Mobile menu button */}
                  <Disclosure.Button className={`${router.pathname !== '/' ? 'text-green-600' : 'text-black-white-200'} 'p-2 rounded-md`}>
                ...
                <div className={`${router.pathname !== '/' ? 'justify-between' : ''} flex items-center`}>
                  {router.pathname === '/' && 
                  <>
                  ...
                  <div className='hidden lgNav:ml-10 xl:ml-32 lgNav:flex lgNav:mt-6'>
                    <div className='flex inset-x-0 top-0 pl-5 invisible lgNav:visible hidden lgNav:inline '>
                      <div className={`${router.pathname === '/' ? 'px-2 sm:px-6 lgNav:px-8' : ''} flex-1 global-margin`}>
                            ...
                            <div className=''>
                              <Popover className='inline-block relative '>
                                {({ open }) => (
                                  <>
                                    <Popover.Button
                                      className={`${router.pathname !== '/' ? 'text-black-white-800' : 'text-white'} group rounded-md inline-flex items-center`}
                                    >
                                      <h5 className='uppercase mobile sm:desktop'>Thema&apos;s</h5>
                                      <ChevronDownIcon
                                        className={`${router.pathname !== '/' ? 'text-gray-400 group-hover:text-gray-500' : 'text-white'} ml-2 h-5 w-5 first-letter
                                        ${open && router.pathname !== '/' ? 'text-gray-500' : 'text-gray-400'}
                                        `}
                                        aria-hidden='true'
                                      />
                                    </Popover.Button>
                              ...
                              <Popover className='inline-block relative '>
                                {({ open }) => (
                                  <>
                                    <Popover.Button
                                      className={`${router.pathname !== '/' ? 'text-black-white-800': 'text-black-white-200'} group rounded-md inline-flex items-center`}
                              ...
                              <div className='inline-block relative'>
                                <Link href={`/about/${encodeURIComponent(FAQslug)}`}>
                                  <h5 className={`${router.pathname !== '/' ? 'text-black-white-800' : 'text-black-white-200'} uppercase pl-8  group rounded-md inline-flex items-center mobile sm:desktop mobile sm:desktop`}>
                                    VRAAG & ANTWOORD
                                  </h5>
                                </Link>
                              </div>
            ...
            <Disclosure.Panel className='lgNav:hidden bg-black-white-200'>
              ...
              <Disclosure.Button as='span' className='flex justify-end items-center pr-8 sm:hidden'>
                <span
