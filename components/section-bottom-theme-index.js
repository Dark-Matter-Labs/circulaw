...
export default function ThemeBottomSection({ props }) {
  const { data } = useSWR(groq`*[_type == "measure" && thema == "${props.thema}" && isFeatured == true]`, (query) => client.fetch(query));
  return (
    <div>
      ...
      <h4 className='urban'>
      Mét voorbeelden en een handige leidraad
      </h4>
      </div>
      ...
      <div className='pt-5 global-margin  '>
        ...
        <div key={measure._id} className='border-b border-grey1 pb-10'>
          ...
          <div className='hidden sm:block pt-6 pr-10'>
              <Image src={urlFor(measure.featuredImage).url()}  alt={measure.featuredImage.altText} width={530} height={256}/>
            </div>
            <div className='col-span-2'>
              <div className='block py-4 sm:hidden'>
              <Image src={urlFor(measure.featuredImage).url()}  alt={measure.featuredImage.altText} width={200} height={200}/>
              </div>
              ...
              <div className='flex justify-between pt-14'>
                ...
                <div className='flex'>
                <div className='border-green1 bg-green1 text-white r-category py-0.5 px-2 rounded-md'>{measure.extraContent[0]}</div>
                <div className='border-green3 bg-green3 text-white r-category p-0.5 px-2 rounded-md ml-4'>{measure.extraContent[1]}</div>
                </div>
                )}
                </div>
                <div>
