            ...
            >
              {data.FAQPageContent.map((item, i) => (
                <Accordion.Item
                  ...
                  value={`item-${i + 1}`}
                >
                  <Accordion.Trigger className='heading-xl-semibold sm:heading-3xl-semibold text-green-600 mr-4 flex justify-between w-full group'>
                    <span className='text-left'>{item.question}</span>{' '}
                    <IconChevronDown className='h-8 w-8 group-data-[state=open]:rotate-180 transition-transform duration-300 ease-in-out' />
                  </Accordion.Trigger>
