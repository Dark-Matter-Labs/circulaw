...
export default async function NewsOverviewPage() {
  const featuresNewsItems = await sanityFetch({query: FEATURED_NEWS_ITEMS_QUERY , tags: ['newsItem']});
  const nonFeaturedNewsItems = await sanityFetch({query: NON_FEATURED_NEWS_ITEMS_QUERY, tags: ['newsItems']});
  return (
    <NewsOverview
