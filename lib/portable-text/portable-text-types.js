    ...
    <Disclosure>
      {({ open }) => (
        <>
          ...
          <Disclosure.Panel>
            <div className='bg-black-white-200 border border-green-500 rounded-lg my-2'>
              <span className='block text-black-white-800 p-4 p-base sm:p-lg'>{value.dropDownTextText}</span>
            </div>
          </Disclosure.Panel>
        </>
      )}
...
export const InlineImageComponent = ({ value }) => {
  ...
  return (
    <div className='relative w-full h-48 lg:h-96 my-6 rounded-md'>
      <Image
        {...imageProps}
