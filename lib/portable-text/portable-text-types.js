...
import YTPlayerClient from './yt-player-client';
  ...
  return (
    <div className=''>
      <div className='rounded-clSm bg-green-500'>
        <div className='gradient-pdf relative z-0 my-10 overflow-hidden rounded-clSm p-10'>
          <div className='rotate-50 invisible absolute -bottom-32 -right-12 -z-10 h-96 w-96 md:visible'>
...
export const InlineImageComponent = (props) => {
  ...
  const imageProps = useNextSanityImage(client, value?.imageFile);
  const aspectRatio =
  ...
  return (
    <div className='relative mb-[60px] w-full rounded-cl sm:mb-[120px]'>
      <Image
        {...imageProps}
...
export const YTComponent = ({ value }) => {
  // Portable Text passes custom types as `value`, not `data`, so this component
  // was reading `undefined.url` and throwing on every page with a YouTube embed.
  // Guard the url too: the studio field is optional, so an empty block is valid.
  if (!value?.url) {
    return null;
  }

  return (
    <div className='mb-[60px] rounded-cl bg-green-100 px-7 py-4 sm:mb-[120px] sm:px-40 sm:py-20'>
      <YTPlayerClient url={value.url} />
    </div>
  );
};
