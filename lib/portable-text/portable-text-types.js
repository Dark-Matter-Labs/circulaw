...
export const dropDownComponent = ({value}) => (
  ...
  <Disclosure>
    ...
    <span className='inline'>{value.dropDownWord}{' '}</span><ChevronUpIcon className={`${open ? '' : 'rotate-180 transform'} h-5 w-5 text-green-300`}/>
    </Disclosure.Button>
    <Disclosure.Panel>
      <div className='bg-black-white-300 my-2'>
      <span className='block text-black-white-800 p-4'>{value.dropDownTextText}</span></div>
    </Disclosure.Panel>
    </>
    )}
  </Disclosure>
