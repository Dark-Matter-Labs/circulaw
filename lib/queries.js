...
// get correct images in thema query - see staging. 
// remove unused props from thema query
export function productChainQueryFunction(productChain) {
  const productChainQueries = {
    ...
    *[_type == "transitionAgenda" && slug.current == "${productChain}"][0] {
    ...
    count(*[_type == "measure" && transitionAgenda == "${productChain}"])
    `,
    themaData: `
    *[_type in ["thema", "simpleThema"] && transitionAgenda == "${productChain}"] {
      ...,
      "heroImage": heroImage.asset->.url,
      "heroImageMobile": heroImageMobile.asset->.url,
      "count": count(*[_type == "measure" && thema == ^.slug.current || simpleThema == ^.slug.current]),
    }
    `
  }
