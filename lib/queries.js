...
export const pcHomePageQuery = `
*[_type == 'transitionAgenda']  | order(order asc) {
  pcName,
  order,
...
// need to rename to make more clear
export function themaQueryFunction() {
  const themaQueries = {
    featured: `
    *[_type == "instrument" && thema->.slug.current == $thema && isFeatured == true]{
      titel,
      slug, 
    ...
    `,
    length: `
    count(*[_type == "instrument" && thema->.slug.current == $thema])
    `,
    themaQuery: `
      *[_type in ["thema", "simpleThema"] && slug.current == $thema][0]{
      ...
      `,
    instrumentsQuery: `
      *[_type == "instrument" && thema->.slug.current == $thema && thema->._type == 'simpleThema']| order(lower(titel) asc) {
        _id,
        titel,
        slug,
        "transitionAgenda": transitionAgenda->.slug.current,
