...
"themesAndProductChains": *[_type == 'transitionAgenda'] | order(order asc) {
  'title': pcName,
  'slug': slug.current,
  'themas': *[_type in ['thema', 'simpleThema'] && transitionAgenda->pcName == ^.pcName] {
    "themaName": themaName,
    "slug": slug.current,
    new,
  } 
...
"newsItems": *[_type == 'newsItem' && featured == true]|order(orderRank) {
...
*[_type == "newsItem" && defined(slug.current) && createPage == true][].slug.current
`;

export const NEWS_DETAIL_PAGE_QUERY = `
*[_type == "newsItem" && slug.current == $slug && createPage == true][0] {
...,
}
`;
...
*[_type == "aboutPage" && slug.current == $slug][0]{
    ...
    aboutPageContent,
    slug,
    "slugs": *[_type == 'aboutPage']|order(orderRank) {
    'slug': slug.current,
    pageTitle,
  },
}
