...
"themesAndProductChains": *[_type == 'transitionAgenda'] | order(orderRank asc) {
  'title': pcName,
  'slug': slug.current,
...
"latestContent": *[_type in ['thema', 'simpleThema', 'euLaw']][0..5] | order(_createdAt desc) {
 ...
 "reactions": *[_type == "reaction"][0..5] | order(_createdAt desc) {
    _id,
    name,
    description
  }
...
// Guards the theme sub-pages (instrumenten, categorie, overheidsbevoegdheid),
// which only exist for full themes and only under their own product chain.
export const FULL_THEME_IN_PRODUCT_CHAIN_QUERY = `
*[_type == 'thema' && slug.current == $thema && transitionAgenda->slug.current == $productChain][0] {
  themaName
}
`;
